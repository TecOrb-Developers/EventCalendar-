...
import static com.appointmentbooking.weekviewNames.WeekViewUtil.daysBetween;
import static com.appointmentbooking.weekviewNames.WeekViewUtil.getPassedMinutesInDay;
import static com.appointmentbooking.weekviewNames.WeekViewUtil.isSameDay;
import static com.appointmentbooking.weekviewNames.WeekViewUtil.today;
...
import com.appointmentbooking.weekviewNames.TextColorPicker;
import com.appointmentbooking.weekviewNames.WeekViewEvent;
import com.appointmentbooking.weekviewNames.WeekViewLoader;
...
public class WeekView extends View {
    ...
    private float mXScrollingSpeed = 0.5f;
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            if (a.getBoolean(R.styleable.WeekView_dropListenerEnabled, false)) this.enableDropListener();
    ...
    private void init() {
        ...
        mNewEventBackgroundPaint.setColor(Color.rgb(245,222,179));
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
       ...
       /*
        // Here is how I added line for every 15 minutes
        int lineCount = (int) ((getHeight() - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / mHourHeight) + 1;
        lineCount = (lineCount) * (mNumberOfVisibleDays + 1) * 4 * (mHourHeight < mMaxHourHeight / 2 ? 1 : 3);
        float[] hourLines = new float[lineCount];*/
        ...
        for(int dayNumber = leftDaysWithGaps + 1;
            ...
            /*int i = 0;  // this code for draw line per 15 mint but getting some issue right now so uncomment upper code
            for (int hourNumber = 0; hourNumber < 12 && hourLines.length > i * 4; hourNumber++) {
                ...
                for (int j = 0; j < (mHourHeight < mMaxHourHeight / 2 ? 1 : 4); ++j, ++i) {
                    hourLines[i * 4] = start;
                    hourLines[i * 4 + 1] = top + mHourHeight * j / 4;
                    hourLines[i * 4 + 2] = startPixel + mWidthPerDay;
                    hourLines[i * 4 + 3] = top + mHourHeight * j / 4;
                }
            }*/
        ...
        for (int dayNumber = leftDaysWithGaps + 1; dayNumber <= leftDaysWithGaps + getRealNumberOfVisibleDays() + 1; dayNumber++) {
            ...
            if (allSpecialistListModel !=null && allSpecialistListModel.getResult() !=null){
                for (int i=0; i<allSpecialistListModel.getResult().size(); i++){
                    Calendar calendar = new GregorianCalendar();
                    calendar.add(Calendar.DATE, i);
                    String dasdd = format.format(calendar.getTime());
                    Log.d("fsf","drawHeaderRowAndEvents: "+dasdd +"\n"+dayLabel);
                  //if (allSpecialistListModel.getResult().get(i).get_id().equalsIgnoreCase(eve))
                    if (dayLabel.equalsIgnoreCase(dasdd)){
                        String names =allSpecialistListModel.getResult().get(i).getName();
                        Log.d("Names", "drawHeaderRowAndEvents: "+allSpecialistListModel.getResult().get(i).getName());
                        canvas.drawText(names, startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding, isToday ? mTodayHeaderTextPaint : mHeaderTextPaint);
                    }
                }
            }else{
                canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2.4f, mHeaderTextHeight + mHeaderRowPadding, isToday ? mTodayHeaderTextPaint : mHeaderTextPaint);
            }
           // String dayLabel = getHeaderValue(0);
            Log.d("dalLable", "drawHeaderRowAndEvents: "+dayLabel +"\n"+day);
           /* if (dayLabel == null)
    ...
    public void setSatus(boolean isStatus){
          this.isStatus = isStatus;
    }

    @RequiresApi(api = Build.VERSION_CODES.N)
    public  String getHeaderValue (int specilistCounter){
        //Collections.sort(allSpecialistListModel.getResult(),Comparator.comparing(Custom.ResultBean::getPosition));
        if (allSpecialistListModel !=null && allSpecialistListModel.getResult() !=null){
            String name = "";
            if(specilistCounter == allSpecialistListModel.getResult().size()) {
                specilistCounter = 0;
                name = allSpecialistListModel.getResult().get(specilistCounter).getName();
            }else{
                name = allSpecialistListModel.getResult().get(specilistCounter).getName();
                specilistCounter+=1;
            }
    ...
    public void setHeader(GetAllSpecialistModel allSpecialistListModel){
        this.allSpecialistListModel = allSpecialistListModel;
    }
            ...
            mDateTimeInterpreter = new DateTimeInterpreter() {
                ...
                public String interpretDate(Calendar date) {
                    try {
                        ...
                        Log.d("interPreDate", "interpretDate: "+sdf.format(date.getTime()).toUpperCase());
     ...
     * </p>
     * @param length Supported values are {@link com.appointmentbooking.weekviewNames.WeekView#LENGTH_SHORT} and
     *               {@link com.appointmentbooking.weekviewNames.WeekView#LENGTH_LONG}.
     */
    @Deprecated
    ...
    public interface EventLongPressListener {
        /**
         * Similar to {@link com.appointmentbooking.weekviewNames.WeekView.EmptyViewClickListener} but with a long press.
         *
         * @param event:     event clicked.
         * @param eventRect: view containing the clicked event.
         */
    ...
    public interface EmptyViewLongPressListener {
        /**
         * Similar to {@link com.appointmentbooking.weekviewNames.WeekView.EmptyViewClickListener} but with long press.
         *
